<?php
  ...
  for ($i = 0; $i < count($old_bike_lst); $i++) {
    ...
    /*if ($old_bike_lst[$i] != $_GET['name'] && $old_bike_lst[$i] != " ") {
  ...
  if ($check_on_service == "f") {
    $update_rack_bikes = "UPDATE mobike.racks SET bike_lst='".$new_bike_lst."' WHERE id='".$_GET['rack']."'";
    $result_rack_update = pg_query($db_connection, $update_rack_bikes);
    $update_bike = "UPDATE mobike.bikes SET res_date='', unlock_token='', onservice='false', reserved='false' WHERE id='".$_GET['name']."'";
